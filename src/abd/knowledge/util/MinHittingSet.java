...
//    protected ABTheoremProver theoremProver = null;
     ...
     */
    public MinHittingSet(boolean conflictsAreMinimal, ArrayList conflictSets) {
        // precondition
...
//    }
     ...
     *
     * Returns an ArrayList of SortedIntList, where each SortedIntList represents a
     * min. diagnosis.
     */
                    ...
                    if (canPrune) {  // prune!
                        ...
                        if ((intersectionSet.size() > 0)        // some subtrees are not relevant
                            ...
                            if (itIntersection.hasNext()) 
                                nextItem = ((PropositionalFormula)itIntersection.next()).hashCode();
                            ...
                            while (itChildren.hasNext()) {
                                ...
                                if (child.edge.hashCode() == nextItem) {
...
//                }
    ...
    protected boolean canClose(HSNode node) {
        // to modify yifan add a condition to check complement in a set
        assert(nodeInvariant(node));
...
class EdgeNodePair {
    ...
    public PropositionalFormula edge;
...
//    }
    ...
    public EdgeNodePair(PropositionalFormula edge, HSNode node) {
        this.edge = edge;
        this.node = node;
    }
